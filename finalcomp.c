...
int main()
{
    runLengthEncoding("venus.bmp", "Output_Image");
...
void runLengthEncoding(char inFileName[], char outFileName[])
{
    ...
    else
    {
        ...
        if (header.bpp != 24)
        {
            printf("Sorry, we can only support BMP images with 24 bits per pixel!\n");
        }
        else
        {
            ...
            /* ------- */
            fseek(inImgFile, 0, SEEK_END);
            fseek(outImgFile, 0, SEEK_END);

            long int inImgSize = ftell(inImgFile);
            long int outImgSize = ftell(outImgFile);
